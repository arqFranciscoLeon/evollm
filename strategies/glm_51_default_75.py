...
class Aggressive_2(LLM_Strategy):
  ...
  def strategy(self, opponent: axl.player.Player) -> axl.Action:
      if self.first_round():
          self._sustain_cooperation = False
          return axl.Action.D
  
      if len(opponent.history) >= 2:
          if (
              self.history[-2:] == [axl.Action.D, axl.Action.D]
              and opponent.history[-2:] == [axl.Action.C, axl.Action.C]
          ):
              self._sustain_cooperation = True
  
      if getattr(self, "_sustain_cooperation", False):
          self._sustain_cooperation = False
          return axl.Action.C
