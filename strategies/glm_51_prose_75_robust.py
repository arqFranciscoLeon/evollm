...
class Neutral_1(LLM_Strategy):
  ...
  def strategy(self, opponent: axl.player.Player) -> axl.Action:
      ...
      if not self.history:
          return axl.Action.C
      # Subsequent rounds: mirror opponent's last move
...
class Neutral_6(LLM_Strategy):
  ...
  def strategy(self, opponent: axl.player.Player) -> axl.Action:
      ...
      # Update count of consecutive mutual defections
      if self.history[-1] == axl.Action.D and opponent.history[-1] == axl.Action.D:
          self.consecutive_mutual_defections += 1
      else:
          self.consecutive_mutual_defections = 0
      
      # Condition 1: If opponent cooperated last round, cooperate
      if opponent.history[-1] == axl.Action.C:
          return axl.Action.C
      
      # Condition 2: If both defected for exactly 2 consecutive rounds, forgive and cooperate
      if self.consecutive_mutual_defections == 2:
          return axl.Action.C
      
      # Condition 3: Otherwise, defect
      return axl.Action.D
...
class Aggressive_17(LLM_Strategy):
  ...
  def strategy(self, opponent: axl.player.Player) -> axl.Action:
      ...
      # Condition 2: I played C last round and opponent played D -> permanent defection
      if self.history[-1] == axl.Action.C and opponent.history[-1] == axl.Action.D:
          self._permanent_defect = True
          return axl.Action.D
  
      # Condition 3: opponent played C last round -> exploit
      if opponent.history[-1] == axl.Action.C:
          return axl.Action.D
  
      # Condition 4: last 10 rounds were mutual defection -> try cooperation
      if len(self.history) >= 10:
      ...
      # Condition 5: all other cases -> defect
      return axl.Action.D
...
class Cooperative_25(LLM_Strategy):
  ...
  def strategy(self, opponent: axl.Player) -> axl.Action:
      ...
      if opponent_last == axl.Action.C:
          return axl.Action.C
      else:  # opponent played D
          if my_last == axl.Action.C:
              return axl.Action.D
          else:  # both played D, forgive
              return axl.Action.C
