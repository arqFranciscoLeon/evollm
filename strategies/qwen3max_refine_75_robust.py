...
class Cooperative_7(LLM_Strategy):
  ...
  def strategy(self, opponent: axl.player.Player) -> axl.Action:
      ...
      # From round 998 onward (0-indexed: when history length >= 997) always defect.
      if len(self.history) >= 997:
          return axl.Action.D
  
      # In rounds 2–997: default to tit-for-tat (copy opponent's previous action).
      # Exception: if both have defected for the last three rounds, cooperate to try to restore mutual cooperation.
      if len(self.history) >= 3:
          if (self.history[-3:].count(axl.Action.D) == 3 and 
                  opponent.history[-3:].count(axl.Action.D) == 3):
              return axl.Action.C
  
      # Default: copy opponent's last move.
      return opponent.history[-1]
...
class Neutral_12(LLM_Strategy):
  ...
  def strategy(self, opponent: axl.player.Player) -> axl.Action:
      if self.first_round():
          return axl.Action.C
      # Rounds 998, 999, 1000: always defect
      if len(self.history) >= 997:
          return axl.Action.D
      # Rounds 2 to 997: Tit-for-Tat
      if opponent.history[-1] == axl.Action.C:
          return axl.Action.C
      else:
          return axl.Action.D
...
class Cooperative_16(LLM_Strategy):
  ...
  def strategy(self, opponent: axl.Player) -> axl.Action:
      ...
      # Rounds 991-1000: always defect
      if round_number >= 991:
          return axl.Action.D
  
      # Rounds 2-990
      # Rule 1: cooperate if opponent cooperated last round
      if opponent.history[-1] == axl.Action.C:
          return axl.Action.C
  
      # Rule 2: otherwise (opponent defected), normally defect
      action = axl.Action.D
  
      # Rule 3: escape mutual defection after 5 consecutive mutual D's
      if round_number >= 6 and \
         all(m == axl.Action.D for m in self.history[-5:]) and \
         all(m == axl.Action.D for m in opponent.history[-5:]):
          action = axl.Action.C
...
class Aggressive_20(LLM_Strategy):
  ...
  def strategy(self, opponent: axl.player.Player) -> axl.Action:
      if self.first_round():
          self._phase = 0                 # 0: normal, 1: punishing
          self._punish_count = 0          # remaining D's in current punishment (excluding the current one)
          self._punish_just_finished = False  # set if we just finished a 3-D block and need to check for test
          return axl.Action.D
  
      # If we just completed a punishment phase, decide whether to test cooperation.
      if self._punish_just_finished:
          self._punish_just_finished = False
          # Only test if the final round of the punishment was mutual defection.
          if opponent.history[-1] == axl.Action.D:
              return axl.Action.C
          # Otherwise fall through to normal processing (the last opponent move was C).
  
      # Rule 1: prioritize exploitation of cooperation.
      if opponent.history[-1] == axl.Action.C:
          self._phase = 0
      ...
      # If we reach here, the opponent defected last round.
      if self._phase == 0:
          # Start a new punishment phase: defect for three rounds (including this one).
          self._phase = 1
          self._punish_count = 2          # after this D, two more to play
          return axl.Action.D
      else:  # self._phase == 1, we are already in a punishment phase
          if self._punish_count > 1:
              self._punish_count -= 1
              return axl.Action.D
          else:
              # This is the last D of the three‑round punishment.
              # Signal that the punishment phase is over; the next round will check for a test.
              self._punish_just_finished = True
              self._phase = 0
              self._punish_count = 0
              return axl.Action.D
