...
class Cooperative_18(LLM_Strategy):
  ...
  @auto_update_score
  def strategy(self, opponent: axl.player.Player) -> axl.Action:
      # First round: cooperate and initialise counter
      if self.first_round():
          self._mutual_defect_count = 0
          return axl.Action.C
      ...
      # Rule 2: If opponent cooperated last round, cooperate and reset counter
      if opp_prev == axl.Action.C:
          self._mutual_defect_count = 0
          return axl.Action.C
  
      # Opponent defected last round
      # Rule 3: Check if we also defected
      if my_prev == axl.Action.D:
          # Mutual defection occurred
          self._mutual_defect_count += 1
          # Rule 4: After exactly 3 mutual defections, cooperate and reset
          if self._mutual_defect_count == 3:
              self._mutual_defect_count = 0
              return axl.Action.C
          else:
              return axl.Action.D
      else:  # my_prev == C, i.e., opponent defected against our cooperation
          # Defect to protect interests, do not change counter
          return axl.Action.D
...
class Cooperative_20(LLM_Strategy):
  ...
  def strategy(self, opponent: axl.player.Player) -> axl.Action:
      ...
      if len(self.history) >= 2:
          if (self.history[-2] == axl.Action.D and self.history[-1] == axl.Action.D and
              opponent.history[-2] == axl.Action.D and opponent.history[-1] == axl.Action.D):
              return axl.Action.C
...
class Cooperative_22(LLM_Strategy):
  ...
  def strategy(self, opponent: axl.player.Player) -> axl.Action:
      if self.first_round():
          return axl.Action.C
      # In each subsequent round, mimic the opponent's previous move
      return opponent.history[-1]
