...
class Aggressive_7(LLM_Strategy):
  ...
  def strategy(self, opponent: axl.player.Player) -> axl.Action:
      if self.first_round():
          return axl.Action.C
      if len(self.history) == 999:  # round 1000
          return axl.Action.D
      # For rounds 2-999, copy opponent's last move
      return opponent.history[-1]
...
class Aggressive_8(LLM_Strategy):
  ...
  def strategy(self, opponent: axl.Player) -> axl.Action:
      if len(self.history) == 0:          # first round
          return axl.Action.D
      elif len(self.history) == 999:      # thousandth round (round 1000)
          return axl.Action.D
      else:                               # rounds 2 through 999
          return opponent.history[-1]
...
class Cooperative_20(LLM_Strategy):
  ...
  @auto_update_score
  def strategy(self, opponent: axl.Player) -> axl.Action:
      # Rule 1: Play Defect in round 1000
      if len(self.history) == 999:
          return axl.Action.D
      # Rule 2: Play Cooperate in round 1
      if self.first_round():
          return axl.Action.C
      # Rule 3: If opponent played Defect in the previous round, play Defect
      if opponent.history[-1] == axl.Action.D:
          return axl.Action.D
      # Rule 4: Otherwise, play Cooperate
      return axl.Action.C
