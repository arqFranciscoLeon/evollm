...
class Cooperative_5(LLM_Strategy):
  ...
  def strategy(self, opponent: axl.player.Player) -> axl.Action:
      ...
      # Round 1000: always defect
      if len(self.history) == 999:
          return axl.Action.D
  
      # Rounds 2 through 999
      # Get the score from the previous round
      last_round_index = len(self.history) - 1
      my_last_move = self.history[last_round_index]
      opp_last_move = opponent.history[last_round_index]
  
      # Determine my score for the last round based on the payoff matrix
      if my_last_move == axl.Action.C and opp_last_move == axl.Action.C:
          ...
          my_score = 5
      else:  # my_last_move == D and opp_last_move == D
          my_score = 1
  
      # Apply the rule
      if my_score in (3, 5):
          return my_last_move
      else:  # my_score in (0, 1)
          return axl.Action.C if my_last_move == axl.Action.D else axl.Action.D
...
class Cooperative_25(LLM_Strategy):
  ...
  def strategy(self, opponent: axl.player.Player) -> axl.Action:
      ...
      # Rule 1: Both defected -> cooperate
      if my_last == axl.Action.D and opp_last == axl.Action.D:
          return axl.Action.C
      # Rule 2: Opponent defected, I cooperated -> defect
      elif opp_last == axl.Action.D and my_last == axl.Action.C:
          return axl.Action.D
      # Rule 3: I defected, opponent cooperated -> return to cooperation
      elif my_last == axl.Action.D and opp_last == axl.Action.C:
          return axl.Action.C
      # Rule 4: Both cooperated -> continue cooperating
      else:  # my_last == C and opp_last == C
          return axl.Action.C
