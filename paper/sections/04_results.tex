...
Nine of twelve model--prompt combinations show $\PC > \PA$, confirming
a widespread cooperative bias. The strongest cooperative equilibria are achieved
...
Noise consistently shifts equilibria toward aggression in all but one
combination. Eleven of twelve model--prompt pairs show
$\PA^{\text{noise}} > \PA^{\text{clean}}$; the exception is Gemini~3.1~Pro~Default,
whose cooperative equilibrium fraction actually rises under noise (see
Section~\ref{sec:noise} and Table~\ref{tab:noise}). GPT-5.4~Mini is the most
noise-resilient: its Default cooperative equilibrium drops from 48\% to 45\%
($\Dnoise = 3$pp). Gemini~2.5~Flash~Prose shows the largest degradation:
...
53--71\%, exceeding the 67\% prior in 10 of 12 cases. Once a system is
seeded with an aggressive majority and action noise is present, convergence
to aggressive equilibria becomes near-certain across all models evaluated.
The two exceptions---Claude~4.6~Default (53\%) and GPT-5.4~Mini~Default
(54\%)---remain at or below the prior.
